...
if __name__ == "__main__":
    d_model = 512
    seq_len = 50
    num_panel_classes = 4  # e.g., triangle, rect, trapezoid, irregular_quad
    num_dialog_shapes = 5  # e.g., oval, explosion, free_text, rect, irregular
    
    # 模拟 LFM 输出
    lfm_output = torch.randn(seq_len, d_model)
    element_types = torch.zeros(seq_len, dtype=torch.long)
    element_types[0] = 0  # PAGE_CTRL
    element_types[1:3] = 1  # 2 Panels
    element_types[3:4] = 2  # 1 Dialog
    element_types[4:5] = 3  # 1 Character
    element_types[5:] = 4  # PAD
    parent_panel_indices = torch.zeros(seq_len, dtype=torch.long)
    parent_panel_indices[3:4] = 1  # Dialog belongs to Panel 1
    parent_panel_indices[4:5] = 1  # Character belongs to Panel 1
    
    # 初始化模型
    model = ParallelPredictionHeads(d_model, num_panel_classes, num_dialog_shapes)
    
    # 前向传播
    panel_outputs, dialog_outputs, character_outputs = model(lfm_output, element_types, parent_panel_indices)
    
    # 模拟目标
    targets = {
        'panel_classes': torch.tensor([0, 1]),  # 2 panels
        'panel_bboxes': torch.tensor([[0.5, 0.5, 0.4, 0.6], [0.7, 0.3, 0.3, 0.2]]),
        'panel_offsets': torch.tensor([[0.0]*8, [0.0]*8]),
        'dialog_bboxes': torch.tensor([[0.6, 0.4, 0.2, 0.1]]),
        'dialog_breakout_labels': torch.tensor([0]),
        'dialog_breakout_ratios': torch.tensor([0.0]),
        'dialog_shapes': torch.tensor([0]),
        'character_bboxes': torch.tensor([[0.5, 0.5, 0.3, 0.5]]),
        'character_breakout_labels': torch.tensor([1]),
        'character_breakout_ratios': torch.tensor([0.05])
    }
    
    # 计算损失
    loss_fn = PredictionLoss()
    total_loss, loss_dict = loss_fn((panel_outputs, dialog_outputs, character_outputs), targets)
